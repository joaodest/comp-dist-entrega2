...
As métricas Prometheus incluem sessões WebSocket, mensagens e bytes por direção, latência de \texttt{PushInput}, erros realtime, tentativas de failover, inputs aceitos/rejeitados, streams ativos, ticks, duração do tick, snapshots descartados, salas, jogadores e eventos de replicação. O Jaeger recebe traces OpenTelemetry das entradas HTTP e chamadas gRPC.
...
O grupo manteve a divisão por fronteiras de serviço, com integração conjunta nos contratos \texttt{.proto}, Docker Compose e testes.

\begin{table}[ht]
\centering
\caption{Divisão de responsabilidades por microsserviço.}
\label{tab:equipes}
\scriptsize
...
Gateway, WebSocket, proxy HTTP, failover do Lobby, health/readiness e métricas da borda. \\ \hline
Letícia, Pedro Augusto e Pedro Henrique &
Lobby, ciclo de vida de sala, \texttt{SetReady}, integração com Game e replicação primário-backup. \\ \hline
André, João e Victor &
Game Service, relógio autoritativo, partidas por sala, regras de arena, cliente web e runner de carga. \\ \hline
Todos os integrantes &
Contratos, Docker Compose, smoke tests, observabilidade, documentação e revisão do relatório. \\ \hline
\end{tabularx}
\end{table}
